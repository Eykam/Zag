...
pub const Eth_Frame = struct {
    ...
    packet_type: [2]u8,
    data: []u8,
    pub fn deinit(self: *Eth_Frame, allocator: std.mem.Allocator) void {
...
test "eth_frame_parsing_minimum_size" {
    ...
    var frame = try Eth_Parser.parse(allocator, &min_frame);
...
test "eth_frame_parsing_undersized_frame_invalid_header" {
    ...
    const frame = Eth_Parser.parse(allocator, &small_frame);
...
test "eth_frame_parsing_undersized_frame_valid_header" {
    ...
    var frame = try Eth_Parser.parse(allocator, &small_frame);
    defer frame.deinit(allocator);
...
test "eth_frame_parsing_maximum_size" {
    ...
    var frame = try Eth_Parser.parse(allocator, &max_frame);
    defer frame.deinit(allocator);
...
test "eth_frame_parsing_oversized_frame" {
    ...
    var frame = try Eth_Parser.parse(allocator, &large_frame);
    defer frame.deinit(allocator);
...
test "eth_frame_parsing_valid_frame_with_specific_values" {
    ...
    var frame = try Eth_Parser.parse(allocator, &valid_frame);
    defer frame.deinit(allocator);
...
test "eth_frame_parsing_valid_frame_with_specific_values_larger" {
    ...
    var frame = try Eth_Parser.parse(allocator, &full_frame);
    defer frame.deinit(allocator);
